...
ggplot(data = mydf, aes(x = reorder(brand, price), y = price)) + geom_bar(stat = "summary", fun  = "mean", fill = "skyblue") +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) #To adjust the "brand names" [taken from "ChatGpt" (line:140)]
```
# Grpah for price and fuel type
...
```
```{r}
ggplot(data = mydf, aes(x = reorder(drivetrain, price), y = price)) + geom_bar(stat = "summary", fun = "mean", fill = "green") + labs(title = "Average price by drivetrain") 
```
```{r}
...
```{r}
# model_3 = lm(mydf$price~mydf$mileage)
# summary(model_3)
```
```{r}
# plot(model_3)
...
model_4 = lm(mydf$price ~ mydf$engine_size + mydf$min_mpg)
model_4
plot(model_4)
```

```{r}
model_5 = lm(mydf$price ~ mydf$drivetrain + mydf$engine_size)
model_5
plot(model_5)
```
